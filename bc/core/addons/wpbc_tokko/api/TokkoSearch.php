...
class TokkoSearch
{
    ...
    function get_url_for_page($page,$request=true){
      if($request){
        return strtok($_SERVER["REQUEST_URI"],'?') ."?".$this->querystring_order_by_key."=".$this->get_search_order_by()."&".$this->querystring_order_key."=".$this->get_search_order()."&".$this->querystring_page_key."=".$page."&limit=".$this->get_current_page_limit()."&data=".json_encode($this->search_data)."";
      }else{
        return "&".$this->querystring_order_by_key."=".$this->get_search_order_by()."&".$this->querystring_order_key."=".$this->get_search_order()."&".$this->querystring_page_key."=".$page."&limit=".$this->get_current_page_limit()."&data=".json_encode($this->search_data)."";
      }
    }
    ...
    function do_search($limit=null, $order_by=null, $order=null){
        if ($this->search_data == null){
            ...
            try {
                ...
                //$url = $this->BASE_SEARCH_URL . $this->querystring_order_by_key."=" . $order_by ."&".$this->querystring_order_key."=". $order ."&format=". $this->results_format ."&key=". $this->auth->key ."&lang=". $this->auth->get_language() ."&limit=". $limit ."&offset=" . $this->get_search_offset() . "&data=" . json_encode($this->search_data);
