...
class WP_Disable_Posts
{
	public function __construct()
	{
		...
		add_filter('wpbc/filter/layout_blog/post_types', array( __CLASS__, 'remove_layout_blog_settings' ),99,1);
		add_filter('acf/wpbc_flex_builder/layouts', array( __CLASS__, 'remove_flex_builder_layouts' ),10,1);
	...
	public static function remove_layout_blog_settings($post_types){ 
		if (($key = array_search('post', $post_types)) !== false) {
		    unset($post_types[$key]);
		} 
		return $post_types;
	}
	public static function remove_flex_builder_layouts($layouts){
		foreach ($layouts as $key => $value) {
			if( $value == 'ui_layout_posts_advanced' || $value == 'ui_layout_taxonomy_advanced' ){
				unset($layouts[$key]);
			}
		}
		return $layouts;
